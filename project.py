...
def compute_local_alignment(seq_x, seq_y, scoring_matrix, alignment_matrix):
    ...
    score = float('-inf')
    for number, matrix_list in enumerate(alignment_matrix):
        dummy_max = max(matrix_list)
        if dummy_max > score:
            score = dummy_max
            length_i = number
    length_j = alignment_matrix[length_i].index(score)
    

    #while length_i !=0 and length_j !=0:     ##old implementation
    while alignment_matrix[length_i][length_j] != 0:
        if alignment_matrix[length_i][length_j] == alignment_matrix[length_i -1][length_j -1] +\
        scoring_matrix[seq_x[length_i - 1]][seq_y[length_j - 1]]:
            #lines 1,2
            secondary_x = seq_x[length_i - 1] + secondary_x
